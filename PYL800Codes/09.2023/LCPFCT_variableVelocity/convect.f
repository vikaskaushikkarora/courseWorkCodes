...
C-----------------------------------------------------------------------
      ...
      NX      =   100   ! Number of cells in the periodic system
      DX      =  0.1   ! Cell size
      DT      =  0.02   ! Timestep for the calculation
      VELX    =  0.5   ! Constant X velocity, VELX*DT/DX = 0.1
      MAXSTP  =  501   ! Number of timesteps, two cycles of the system
      LOUT    = stdout   ! Logical unit number of printed output device
      IPRINT  =  50   ! Printout frequency, fluid moves  cells
	  SEEFL	  = VELX*DT/DX
...
C-----------------------------------------------------------------------
         If ( MOD(ISTEP-1,IPRINT) .eq. 0 ) Then
            JSTEP = ISTEP - 1
            Write ( LOUT, 1000 ) JSTEP, TIME, DX, DT, VELX, SEEFL
            Write ( LOUT, 1001 )
...
C-----------------------------------------------------------------------
      ...
      Data       HEIGHT, X0, WIDTH / 1.0, 10.0, 5.0 /
